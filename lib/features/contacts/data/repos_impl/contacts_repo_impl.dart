...
class ContactsRepoImpl extends ContactsRepo {
  ...
  Future<Either<Failure, List<UserWithContactStatusEntity>>> searchInUsers(
    ...
    try {
      ...
      final users = (result[ApiKeys.users] as List).map((userJson) {
        final userEntity = UserModel.fromJson(userJson).toEntity();
        final isContact = (userJson[ApiKeys.isContact] ?? -1) >= 0;
  ...
  @override
  Future<Either<Failure, List<UserWithContactStatusEntity>>>
      getUserContacts() async {
    try {
      final result = await apiConsumer.get(
        EndPoints.getUserContacts,
      );

      final users =
          (result[ApiKeys.users][ApiKeys.contacts] as List).map((userJson) {
        final userEntity = UserModel.fromJson(userJson).toEntity();
        final isContact = true;
        return UserWithContactStatusEntity(
          user: userEntity,
          isContact: isContact,
        );
